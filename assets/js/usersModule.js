import { SUPABASE_ANON_KEY, SUPABASE_URL, supabaseDb } from "./supabaseClient.js";
...
function resolveSelectors() {
  selectors = {
    ...
    passwordField: document.querySelector("#user-password"),
    activeCheckbox: document.querySelector("#user-active")
  };
}

const CREATE_USER_FUNCTION_URL = `${SUPABASE_URL}/functions/v1/crear_usuario`;
...
function registerEventListeners() {
  ...
  selectors.userForm?.addEventListener("submit", async (event) => {
    ...
    if (
      ...
      || !payload.correo
      || !payload.area_id
      || !payload.jerarquia_id
      || (!state.editingUser && !payload.password)
    ) {
...
function openDialog(title, user = null) {
  ...
  if (selectors.passwordField) selectors.passwordField.value = "";
  ...
  if (user) {
    ...
    if (selectors.passwordField) selectors.passwordField.placeholder = "Solo necesario al crear";
  } else if (selectors.passwordField) {
    selectors.passwordField.placeholder = "Ingresa una contraseña temporal";
  }
...
async function callCrearUsuarioFunction(payload) {
  try {
    ...
    if (!response.ok || !result?.ok) {
      const message = result?.error || `No se pudo crear el usuario (HTTP ${response.status}).`;
...
async function createUser(payload) {
  try {
    const userId = await callCrearUsuarioFunction(payload);

    if (!userId) {
      setDialogHint("No se obtuvo un ID de usuario válido.", true);
      return;
    }
  ...
  try {
    ...
    if (payload.rol_id) {
      const { error: roleError } = await supabaseDb
        .from("usuarios_roles")
        .insert([{ usuario_id: userId, rol_id: payload.rol_id }]);
