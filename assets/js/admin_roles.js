...
async function loadRoles() {
  ...
  const columns = state.supportsActivo ? "id,nombre,descripcion,activo,created_at" : "id,nombre,descripcion,created_at";
  ...
  if (error) {
    const missingColumnMessage = String(error.message ?? "").toLowerCase();
    if (missingColumnMessage.includes("column") && missingColumnMessage.includes("activo")) {
      state.supportsActivo = false;
      updateActivoAvailability();
      return loadRoles();
    }
...
function applyFilters(roles) {
  ...
  const statusFilter = state.supportsActivo ? state.filters.status : "todos";
  ...
  return roles.filter((role) => {
    ...
    const activo = state.supportsActivo ? role.activo ?? true : true;
    const matchesStatus =
...
function registerEventListeners() {
  ...
  selectors.roleForm?.addEventListener("submit", async (event) => {
    ...
    if (state.supportsActivo) {
      payload.activo = selectors.roleActiveCheckbox?.checked ?? true;
    }
...
function openDialog(title, role = null) {
  ...
  if (selectors.roleActiveCheckbox && state.supportsActivo) selectors.roleActiveCheckbox.checked = true;
  ...
  if (role) {
    ...
    if (selectors.roleActiveCheckbox && state.supportsActivo) {
...
async function createRole(payload) {
  try {
    ...
    let { error } = await supabaseDb.from("roles").insert([insertPayload]);
    if (error) {
      const missingColumnMessage = String(error.message ?? "").toLowerCase();
      if (missingColumnMessage.includes("column") && missingColumnMessage.includes("activo")) {
        state.supportsActivo = false;
        updateActivoAvailability();
        delete insertPayload.activo;
        ({ error } = await supabaseDb.from("roles").insert([insertPayload]));
      }
    }
...
async function updateRole(roleId, payload) {
  try {
    ...
    let { error } = await supabaseDb.from("roles").update(updatePayload).eq("id", roleId);
    if (error) {
      const missingColumnMessage = String(error.message ?? "").toLowerCase();
      if (missingColumnMessage.includes("column") && missingColumnMessage.includes("activo")) {
        state.supportsActivo = false;
        ...
        ({ error } = await supabaseDb.from("roles").update(updatePayload).eq("id", roleId));
      }
    }
...
function updateActivoAvailability() {
  const statusGroup = selectors.statusSelect?.closest(".input-group");
  if (selectors.statusSelect) {
    if (state.supportsActivo) {
      selectors.statusSelect.disabled = false;
      if (statusGroup instanceof HTMLElement) statusGroup.style.display = "";
    } else {
      selectors.statusSelect.disabled = true;
      selectors.statusSelect.value = "todos";
      state.filters.status = "todos";
      if (statusGroup instanceof HTMLElement) statusGroup.style.display = "none";
    }
  }

  const activeField = selectors.roleActiveCheckbox?.closest(".form__field");
  if (activeField instanceof HTMLElement) {
    activeField.style.display = state.supportsActivo ? "" : "none";
  }
}
